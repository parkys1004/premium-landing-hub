  ...
  return (
    <section className="relative">
      ...
      >
        ...
        <div className="relative bg-white dark:bg-slate-900/80 rounded-[22px] p-8 md:p-12 transition-colors duration-300">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-12 border-b border-slate-100 dark:border-slate-800 pb-8">
            <div className="flex items-center gap-5">
              <div className="p-4 rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 ring-1 ring-indigo-500/10 dark:ring-indigo-500/30">
                <Mail size={28} />
              </div>
              <div>
                <h3 className="text-3xl font-bold text-slate-900 dark:text-white">자료 요청서</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {formItems.map((item, idx) => (
              <motion.div
                ...
                className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group/card"
              >
                <div className={`w-10 h-1.5 mb-4 rounded-full bg-gradient-to-r ${item.color} opacity-70`}></div>
                <h4 className="font-bold text-slate-900 dark:text-slate-200 text-xl mb-2">{item.title}</h4>
                <p className="text-base text-slate-600 dark:text-slate-400 mb-4">{item.desc}</p>
                <div className="text-sm text-indigo-700 dark:text-indigo-300/80 bg-white dark:bg-indigo-950/30 p-3 rounded-lg border border-slate-200 dark:border-indigo-500/20 italic">
                  {item.eg}
                </div>
              </motion.div>
            ))}
          ...
          <motion.div 
             ...
             className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 mb-10 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
             <div className="w-10 h-1.5 mb-4 rounded-full bg-gradient-to-r from-gray-400 to-slate-600 opacity-70"></div>
             <h4 className="font-bold text-slate-900 dark:text-slate-200 text-xl mb-2">5. 이미지 및 로고 파일</h4>
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.98 }}
            onClick={handleCopy}
            className={`w-full relative overflow-hidden py-6 rounded-xl font-bold text-xl shadow-lg flex items-center justify-center gap-3 transition-all duration-300
              ${copied 
