        ...
        <div className="flex flex-col md:flex-row gap-10 items-start">
          ...
          <div className="flex-1">
            ...
            <div className="space-y-10">
              ...
              />
              <NoticeItem 
                icon={<Lock className="text-emerald-500 dark:text-emerald-400" size={24} />}
                title="보안은 어떻게 유지되나요?"
          ...
          <div className="w-full md:w-5/12">
            <motion.div 
              ...
              className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-500/20 rounded-2xl p-8 relative overflow-hidden transition-colors"
            >
              <div className="absolute top-0 left-0 w-1.5 h-full bg-amber-500"></div>
              <div className="flex gap-5">
                <div className="shrink-0 mt-1 p-2.5 bg-amber-100 dark:bg-amber-500/10 rounded-xl text-amber-600 dark:text-amber-500">
                  <AlertCircle size={28} />
                </div>
                <div>
                  <h4 className="font-bold text-slate-800 dark:text-amber-100 mb-3 text-lg">이메일 인증 코드 (중요)</h4>
                  <p className="text-base text-slate-600 dark:text-amber-200/70 leading-relaxed">
...
const NoticeItem = ({ icon, title, text }: any) => (
  <div className="flex gap-5 group">
    <div className="shrink-0 mt-1 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 group-hover:bg-slate-100 dark:group-hover:bg-slate-800 transition-colors">
      {icon}
    </div>
    <div>
      <h4 className="font-bold text-slate-800 dark:text-slate-200 mb-2 text-xl">{title}</h4>
